...
def create_person_table(connection, id:int, score:int, variant):
    """
    Create and insert roaddanger-dehumanisation table
    ...
    # Create the `persons` table
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS roaddanger_dehumanisation (
            id INTEGER PRIMARY KEY,
            dehumanisation INTEGER NOT NULL,
    ...
    # Insert data into the `persons` table
    person_query = """
        INSERT INTO roaddanger_dehumanisation (id, dehumanisation, variant)
        VALUES (?, ?, ?)
    """
